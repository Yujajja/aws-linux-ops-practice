...
AWS EC2 기반 Linux 서버 운영 환경을 구성하고, Ansible을 활용해 서버 초기 설정을 자동화한 실습 프로젝트입니다.

EC2 2대를 서로 다른 가용 영역에 배치한 뒤 Application Load Balancer와 Target Group을 구성하여, 한 서버에 장애가 발생해도 정상 서버를 통해 서비스가 유지되는 흐름을 확인했습니다.

또한 Shell Script로 서버 상태 점검 항목을 표준화하고, CloudWatch Alarm과 Runbook을 함께 구성하여 장애 감지부터 1차 복구까지의 운영 대응 절차를 정리했습니다.
...
이번 실습을 통해 단일 EC2 서버 구성은 장애 발생 시 서비스 중단 위험이 크기 때문에, 최소 2대 이상의 서버를 서로 다른 가용 영역에 배치하고 ALB를 통해 트래픽을 분산하는 구조가 필요하다는 것을 이해했습니다.

또한 Ansible Playbook을 활용해 반복적인 서버 설정 작업을 자동화하고, Shell Script로 서버 상태 점검 항목을 표준화하면서 Linux 서버 운영 업무에서 자동화가 왜 중요한지 경험했습니다.

단순히 서버를 구축하는 것에서 끝내지 않고, ALB Health Check와 CloudWatch Alarm을 통해 장애를 감지하고 Runbook으로 복구 절차를 정리하면서 운영자의 관점에서 장애 1차 대응 흐름을 실습했습니다.
